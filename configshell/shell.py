...
class ConfigShell:
    ...
    def _display_completions(self, substitution, matches, max_length):  # noqa: ARG002 TODO
        ...
        self.con.raw_write("\n")
        if matches:
            nr_cols = width // max_length if max_length < width else 1
    ...
    def _complete_token_command(self, text, path, command):  # noqa: ARG002 TODO
        '''
        ...
        completions = [command + ' ' if len(completions) == 1 else command for command in commands
    ...
    def _complete_token_path(self, text):
        ...
        if names and partial_name in ('', '*'):  # noqa SIM102 - readability
        ...
        if len(completions) == 1:
            ...
            if not completions[0].endswith("* "):  # noqa SIM102 - readability
    ...
    def _complete_token_pparam(self, text, path, command, pparams, kparams):
        ...
        current_parameters = dict(kparams)
        ...
        if pparam_ok:
            pparam_index = len(pparams) if not text else len(pparams) - 1
    ...
    def _complete_token_kparam(self, text, path, command, pparams, kparams):
        ...
        for index in range(len(pparams)):
            current_parameters[cmd_params[index]] = pparams[index]
        current_parameters = dict(kparams.items())
        completion_method = target.get_completion_method(command)
        if completion_method:
    ...
    def _execute_command(self, path, command, pparams, kparams):
        ...
        result = None
        targets = [target] if not iterall else target.children
    ...
    def run_interactive(self):
        ...
        if history and index and index < len(history):
            try:
